...
from _utils import *
from ddp_utils import is_main_process, get_tqdm

logger = logging.getLogger(__name__)

def load_dataset_from_examples(args, examples, pool, tokenizer, split_tag, only_src=False):
    ...
    if split_tag == 'test' or only_src:
        ...
        if args.target_update_manner in ["none"]:
            all_target_ids = torch.tensor([f.target_ids for f in features], dtype=torch.long)
            data = TensorDataset(all_source_ids, all_target_ids)
        elif args.target_update_manner in ["first_epoch", "epoch", "step"]:
            all_indices = torch.tensor([f.example_id for f in features], dtype=torch.long)
            data = TensorDataset(all_source_ids, all_indices)
        else:
...
def load_and_cache_gen_data(args, filename, pool, tokenizer, split_tag, only_src=False, is_sample=False):
    ...
    if isinstance(filename, str):
        language = "ruby" if "ruby" in filename else "javascript" if "javascript" in filename else "go" if "go" in filename else "python" if "python" in filename else "java" if "java" in filename else "php" if "php" in filename else None
        assert language is not None
    data_tag = '_all' if args.data_num == -1 else '_%d' % args.data_num
    if language is not None:
        data_tag = "_{}{}".format(language, data_tag)
    cache_fn = '{}/{}.pt'.format(args.cache_path, split_tag + ('_src' if only_src else '') + data_tag)

    examples = read_examples(filename, args.data_num, args.task, split_tag, args)
    #if isinstance(filename, str):
    #    examples = read_examples(filename, args.data_num, args.task)
    #elif isinstance(filename, list):
    #    examples = []
    #    for fname in filename:
    #        examples.extend(read_examples(fname, args.data_num // len(filename), args.task))

    if is_sample:
        examples = random.sample(examples, min(5000 * len(filename), len(examples)))
    if split_tag == 'train':
    ...
    if os.path.exists(cache_fn) and not is_sample:
        ...
        data = torch.load(cache_fn, map_location=map_location)
    else:
        if is_main_process():
            if is_sample:
                logger.info("Sample %dk data for computing bleu from %s", 5 * len(filename), filename)
                #logger.info("Sample 5k data for computing bleu")
            else:
        ...
        if only_src:
            ...
            if args.target_update_manner in ["first_epoch", "epoch", "step"] and split_tag in ["train"]:
                all_indices = torch.tensor([f.example_id for f in features], dtype=torch.long)
                data = TensorDataset(all_source_ids, all_indices)
            #elif args.target_update_manner in ["none"] or split_tag in ["dev"]:
            else:
                all_target_ids = torch.tensor([f.target_ids for f in features], dtype=torch.long)
                data = TensorDataset(all_source_ids, all_target_ids)
        if is_main_process() and not is_sample:
        #if args.local_rank in [-1, 0] and not is_sample:
            torch.save(data, cache_fn)
    assert len(examples) == len(data), "len(examples) = {}, len(data) = {}".format(len(examples), len(data))
...
def load_and_cache_clone_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if os.path.exists(cache_fn):
        ...
        features = pool.map(convert_clone_examples_to_features, get_tqdm(tuple_examples))
        all_source_ids = torch.tensor([f.source_ids for f in features], dtype=torch.long)
...
def load_and_cache_defect_data(args, filename, pool, tokenizer, split_tag, is_sample=False):
    ...
    if os.path.exists(cache_fn):
        ...
        features = pool.map(convert_defect_examples_to_features, get_tqdm(tuple_examples))
...
def get_filenames(data_root, task, sub_task, split=''):
    if task == 'concode':
        ...
        if sub_task == "multi":
            ...
            for s_task in ["ruby", "javascript", "go", "python", "java", "php"]:
                data_dir = '{}/{}/{}'.format(data_root, task, s_task)
                train_fn.append('{}/train.jsonl'.format(data_dir))
                dev_fn.append('{}/valid.jsonl'.format(data_dir))
                test_fn.append('{}/test.jsonl'.format(data_dir))
        else:
...
def read_examples(filename, data_num, task, split_tag, args=None):
    read_example_dict = {
    ...
    return read_example_dict[task](filename, data_num, split_tag, args=args)
